...
func NewCUpdateCommand(dockerCli command.Cli) *cobra.Command {
	var options CupdateOptions

	cmd := &cobra.Command{
		Use:   "Cupdate [OPTIONS] CONTAINER [CONTAINER...]",
		Short: "Update configuration of one or more containers  (test) ",
		Args:  cli.RequiresMinArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			options.containers = args
			options.nFlag = cmd.Flags().NFlag()
			return runCUpdate(dockerCli, &options)
		},
	}
...
func runCUpdate(dockerCli command.Cli, options *CupdateOptions) error {
	var err error
